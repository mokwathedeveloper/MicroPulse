...
class ProductServiceApp {
  ...
  public async start(): Promise<void> {
    try {
      ...
      // Start HTTP server
      const port = parseInt(process.env.PRODUCT_SERVICE_PORT || '8002');
      ...
      this.server = this.app.listen(port, () => {
        ...
        logger.info(`Static files: Serving from /uploads`);
      });
...
const productService = new ProductServiceApp();
productService.start().catch((error) => {
  logger.error('Failed to start Product Service:', error);
  process.exit(1);
});

export default ProductServiceApp;
